...
document.addEventListener('DOMContentLoaded', function () {
  const scene = document.getElementById('toolbar')
  let toolBar = toolbar()
  ...
  toolBar.add(createDiamondNode, scene, (state) => {
  ...
  toolBar.add(createCircleNode, scene, (state) => {
...
function toolbar() {
  ...
  return {
    ...
    add: (nodeToDraw, scene, functionThatChangesState) => {
      if (find(nodeToDraw) === null) {
        if (buttons.length > 0) {
          ...
          const button = createButton(x, y, nodeToDraw, scene, functionThatChangesState)
          buttons.push(button)
        } else {
          const dim = scene.getBoundingClientRect()
...
function createButton(x, y, nodeToDraw, scene, aStrategy) {
